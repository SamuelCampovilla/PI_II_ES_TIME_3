...
document.addEventListener('DOMContentLoaded', async() => {
        ...
        try{
            ...
            if (resposta.ok && data.instituicoes) { 
               data.instituicoes.forEach(institution => {
                    const cursosCount = 0; // se quiser buscar real, adaptar
                    ...
                    card.innerHTML = `
                        <div class="institution_info">
                            ...
                            <button class="btnExcluir" data-id="${institution.id_instituicao}" data-name="${institution.nome_instituicao}" type="button">
    ...
    // importa modal (deve expor função para abrir, mas também manipulamos DOM diretamente)
    ...
    document.addEventListener('click', async (event) => {
        ...
        // cancelar: fecha modal e remove listener
        if (cancelButton) {
            const cancelHandler = () => {
                if (confirmButton) confirmButton.removeEventListener('click', confirmHandler);
                closeExclusionModal();
